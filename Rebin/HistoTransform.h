...
class HistoTransform 
{
    ...
    public:
        ...
        float trafoFzSig;
        float trafoFzBkg;
        float trafoSixMCstatUpBound;
        float trafoFMCstatUpBound;
        float trafo14MinFracError;
        float trafo14MinBkg;
...
HistoTransform::HistoTransform() 
{
    ...
    trafoSixMCstatUpBound = 1.; // sigma_b cut, 1. = no cut
    trafoFMCstatUpBound = 1.;
    trafo14MinFracError = 0.01;
    trafo14MinBkg = 10;
}
...
std::vector<int> HistoTransform::getRebinBins(TH1* histoBkg, TH1* histoSig, int method, double maxUnc) 
{
    std::vector<int> bins;
    bins.clear();

    if (!histoBkg || (!histoSig && method !=13) ) {
    ...
    float nBkg = histoBkg -> IntegralAndError(0, nBins + 1,nBkgerr);


    //This is used for method 13. A constant background based on total statistical error on background
    //
    float NinNewBin=(nBkgerr*nBkgerr)/(maxUnc*maxUnc*nBkg);

    ////Estimate the number of bins after
    //float nnewbins = NinNewBin>0 ?  nBkg/NinNewBin : 0;
    ////Prevent having too many bins
    //if(nnewbins>30) NinNewBin = nBkg/30.0 ;

    float nSig = (histoSig)?  histoSig -> Integral(0, nBins + 1) : 0;

    double nLLRs = (histoSig)? getLLR(histoSig, histoBkg) : 0;
    double nLLRb = (histoSig)? getLLR(histoBkg, histoSig) : 0;
    ...
    while (iBin > 0) {
        ...
        while (!pass && iBin >= 0) {
            ...
            double nSigBin = (histoSig)? histoSig -> GetBinContent(iBin) : 0;
            sumBkg += nBkgBin;
            sumSig += nSigBin;
            ...
            switch (method) {
                ...
                case 6:
                    ...
                    pass = sqrt(err2Rel) < 1 && sqrt(err2RelBkg) < trafoSixMCstatUpBound;
                    // distance
                    dist = fabs(err2Rel - 1);
                    break;
                case 7:
                ...
                case 12:
                    ...
                    pass = sqrt(err2Rel) < 1 && sqrt(err2RelBkg) < trafoFMCstatUpBound;
                    break;
                case 13:
                    //	  A constant background based on total statistical error on background
                    pass=sumBkg>NinNewBin;
                    //std::cout << "ZEBRA " << sumBkg << " " << NinNewBin << " " << pass << std::endl;
                    break;
                case 14:
                    // Relative bkg error less than maxUnc weighted by fraction of signal in bin (with 1% min if no signal in bin) + 10 min bkg events
                    if (nSig > 0)
                        pass = sqrt(err2RelBkg) < maxUnc * (sumSig/nSig) + trafo14MinFracError && sumBkg>trafo14MinBkg;
                    else 
                        pass = sqrt(err2RelBkg) < maxUnc && sumBkg>trafo14MinBkg;
                    break;
...
void HistoTransform::rebinHisto(TH1* histo, std::vector<int>* bins, bool equidist, bool relabel) 
{
